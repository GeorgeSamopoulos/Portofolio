...
public class LoginAdmin extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String result = ed.getDoctorsNotCert();
            HttpSession session = request.getSession();
            session.setAttribute("name", "admin");
            session.setAttribute("pass", "admin12*");
            result += ed.getDoctors();
            EditSimpleUserTable eu = new EditSimpleUserTable();
            result += eu.getUsers();
            result += "<button onclick=\"LogOut();\">Log out</button>";
            out.println(result);
